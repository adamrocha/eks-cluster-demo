resource "tls_private_key" "ingress_ssl_key" {
  algorithm = "RSA"
  rsa_bits  = 2048
}

resource "tls_self_signed_cert" "ingress_ssl_cert" {
  private_key_pem = tls_private_key.ingress_ssl_key.private_key_pem

  subject {
    common_name  = "hello-world-ingress.local"
    organization = "eks-cluster-demo"
  }

  ip_addresses = [
    "127.0.0.1"
  ]

  allowed_uses = [
    "key_encipherment",
    "digital_signature",
    "server_auth"
  ]

  validity_period_hours = 8760
  is_ca_certificate     = false
}

resource "aws_acm_certificate" "ingress_ssl_cert" {
  private_key      = tls_private_key.ingress_ssl_key.private_key_pem
  certificate_body = tls_self_signed_cert.ingress_ssl_cert.cert_pem

  tags = {
    Name = "ingress-ssl-cert"
  }

  lifecycle {
    create_before_destroy = true
  }
}